...
| [implement](skills/engineering/implement/SKILL.md) | You or model | Implement a GitHub issue end-to-end: fetch → explore → BDD → validate → open a PR. |
...
- **`implement` is a deeper flow than Matt's was.** Matt's `implement` was a thin wrapper; this fork replaced it with an end-to-end flow — fetch → explore → BDD → self-review → validate → PR, plus a fresh-reviewer pass — that was called `work-on-issue` before reclaiming the `implement` name.
