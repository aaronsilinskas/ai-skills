...
argument-hint: "the bug, failure, or slow path to diagnose"
---
...
**Perf branch.** For performance regressions, logs are usually wrong. Instead: establish a baseline measurement (timing harness, `time.perf_counter()`, a profiler such as `cProfile`, query plan), then bisect. Measure first, fix second.
