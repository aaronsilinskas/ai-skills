# Whole-Diff Smell Baseline

The structural smells to check across an entire change (Fowler, _Refactoring_,
ch. 3). This is the **review altitude**: smells that only surface once you can
see the whole diff against a fixed point. The cheaper, local subset — extract
the duplication you just made, rename what you just realised is misnamed —
belongs to the under-green refactor beat in the `bdd` loop (see
`bdd/refactoring.md`); this list is the deeper pass.

Two rules bind every entry:

- **The repo overrides.** A documented repo standard always wins; where it
  endorses something this list would flag, suppress the smell.
