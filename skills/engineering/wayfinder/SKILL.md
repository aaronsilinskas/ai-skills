...
Blocking uses a **`Blocked by #x` body convention** — GitHub has no native dependency link, so the frontier is computed, not rendered (see the project's Wayfinding operations, or the [github-operations.md](github-operations.md) default). A ticket is **unblocked** when every ticket blocking it is closed; the **frontier** is the open, unblocked, unclaimed children — the edge of the known.
