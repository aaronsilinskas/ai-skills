...
For each approved ticket, publish a new issue to the issue tracker. Use the issue body template below. These tickets are ready for AFK agents, so publish them with the correct triage label unless instructed otherwise.

Publish tickets in dependency order (blockers first) so each ticket's blocking edges can reference real issue identifiers. Express hierarchy and blocking using the tracker's conventions — attach each issue under the parent as a sub-issue, and record blockers with the `Blocked by #<n>` convention (see the tracker doc's **Issue relationships** section; GitHub has no native issue-to-issue blocking, so blocking is the body convention).
...
A concise description of this ticket. Describe the end-to-end behavior, not layer-by-layer implementation.
...
- `Blocked by #<n>` for each blocking ticket, or "None — can start immediately".
